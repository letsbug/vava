...
import { ITypeStatisticsGender, ITypeStatisticsArea, ITypeStatisticsTraffic, ITypeStatisticsBasic } from '@/apis/types';
 ...
 * @returns {number}
 */
function generateUV(pv: number): number {
  const min = 500;
 ...
 */
export function generatePV(date: number): ITypeStatisticsBasic {
 ...
 * @param pv
 */
export function generateAreas(pv: number): ITypeStatisticsArea[] {
  const areas: any = [];
  ...
  areas.sort((a: any, b: any) => b.value - a.value);
  return areas;
}
 ...
 */
export function generateGender(uv: number): ITypeStatisticsGender {
 ...
 * @returns {Array}
 */
export function generateAges(uv: number) {
  const ages: any[] = [];
 ...
 * @param pv
 */
export function generateTraffics(pv: number): ITypeStatisticsTraffic[] {
  const traffics: ITypeStatisticsTraffic[] = [];
  const source: any[] = [];
  const interview: any[] = [];
  ...
  Dictionaries.originSite.forEach((url, i) => {
    traffics.push({
      url,
      pageviews: Math.floor(Math.random() * average + average),
      type: 'source'
    });
    traffics.push({
      url: Dictionaries.pages[i],
      pageviews: Math.floor(Math.random() * average + average),
      type: 'interview'
    });
  });
