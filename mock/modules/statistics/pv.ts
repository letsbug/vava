 ...
 */
export function generateGender(uv: number) {
 ...
 * @param pv
 */
export function generateAreas(pv: number) {
  const areas: any = [];
 ...
 * @param pv
 */
export function generateTraffics(pv: number) {
  const source: any[] = [];
  const interview: any[] = [];
  ...
  Dictionaries.originSite.forEach((url, i) => {
    source.push({
      url,
      pv: Math.floor(Math.random() * average + average)
    });
    interview.push({
