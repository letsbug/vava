...
const mergeRangeTypes = {
  ...
  '=-': function (mergeResult, mergeIndex, compareResult, compareIndex) {
    ...
    if (mep2 > cep1) {
      const length = compareRange.fragment.length;
      let offset = 0;
      if (mergeRange[fields.version] !== fields.initialVersion) {
        // 非初始版本
...
class IPaintedFace {
  ...
  compare(leftVersion, rightVersion) {
    ...
    while (leftPos < len1 && rightPos < len2) {
      ...
      if (!range) {
        // 修改模式；左右都找不到，说明左侧内容被删除，同时又添加了新内容
        if (modifyLeftPos === -1) {
          // 记录开始位置
      ...
      if (leftPos !== range.sp1 && rightPos === range.sp2) {
        // 普通删除
        ranges.push({
          type: '-',
          sp1: leftPos,
