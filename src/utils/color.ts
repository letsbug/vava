...
export function rgb2Hex(rgb: string) {
  const _: string[] = rgb.split(',');
  const r = parseInt(_[0].split('(')[1]);
  const g = parseInt(_[1]);
  const b = parseInt(_[2].split(')')[0]);
  return '#' + ((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1);
}

export function hex2Rgb(hex: string, onlyNum?: boolean) {
